...
def validate(package_root: Path, allow_building: bool) -> dict[str, Any]:
    ...
    if not package_root.is_dir():
        return {
            "valid": False,
            "errors": [f"Pakketmap bestaat niet: {package_root}"],
            "warnings": [],
            "checks": {},
        }
    for value in required:
        if not (package_root / value).is_file():
            errors.append(f"Verplicht bestand ontbreekt: {value}")
    if errors:
    ...
    try:
        ...
        package = {}
        errors.append(f"package.json is ongeldig: {exc}")
        ...
        with hash_manifest_path.open("r", encoding="utf-8", newline="") as handle:
            ...
            for row_number, row in enumerate(reader, start=2):
                ...
                if not valid_relative_path(relative_path):
                    errors.append(f"Ongeldig relatief pad in sha256.csv regel {row_number}: {relative_path!r}")
                    continue
                ...
                try:
                    ...
                    errors.append(f"Ongeldige bestandsgrootte voor {relative_path}")
                    continue
                digest = row.get("sha256", "").lower()
                if len(digest) != 64 or any(character not in "0123456789abcdef" for character in digest):
                    errors.append(f"Ongeldige SHA-256 voor {relative_path}")
                    continue
    ...
    for value in sorted(actual_files - listed_files):
        errors.append(f"Onverwacht/niet-gehasht bestand: {value}")
    for value in sorted(listed_files - actual_files):
        errors.append(f"Gehashte file ontbreekt: {value}")
    checked_hashes = 0
    ...
    for forbidden in FORBIDDEN_CODEX_PATHS:
        if forbidden.lower() in actual_all:
            errors.append(f"Machinegebonden Codex-bestand aangetroffen: {forbidden}")
    for forbidden_directory in FORBIDDEN_CODEX_DIRECTORIES:
        ...
        if any(value == forbidden_directory.lower() or value.startswith(prefix) for value in actual_all):
            errors.append(f"Machinegebonden Codex-map aangetroffen: {forbidden_directory}")
    ...
    try:
        ...
        try:
            ...
            if "threads" not in tables:
                errors.append("Snapshotdatabase bevat geen tabel threads.")
            else:
                database_threads = {
    ...
    try:
        ...
        thread_manifest = []
        errors.append(f"threads.json is ongeldig: {exc}")
    ...
    for item in thread_manifest:
        ...
        if not isinstance(relative_path, str) or not valid_relative_path(relative_path):
            errors.append(f"Thread {thread_id} heeft geen geldig backupRelativePath.")
            continue
        if relative_path in referenced_rollouts:
            errors.append(f"Rolloutbestand door meer dan één thread gebruikt: {relative_path}")
        referenced_rollouts.add(relative_path)
        rollout = package_root / Path(*PurePosixPath(relative_path).parts)
        if not rollout.is_file():
            errors.append(f"Rolloutbestand ontbreekt voor thread {thread_id}: {relative_path}")
            continue
        rollout_id, rollout_error = parse_session_id(rollout)
        if rollout_error:
            errors.append(f"Ongeldige rollout {relative_path}: {rollout_error}")
    ...
    if orphan_files:
        warnings.append(f"{len(orphan_files)} orphan rolloutbestand(en) aanwezig.")
    ...
    if declared_sessions != len(session_files):
        errors.append(
            f"counts.sessionFiles ({declared_sessions}) wijkt af van bestanden ({len(session_files)})."
        )
    ...
    try:
        ...
        projects = []
        errors.append(f"projects.json is ongeldig: {exc}")
    for item in projects:
        ...
        relative_path = item.get("backupRelativePath")
        if not isinstance(relative_path, str) or not valid_relative_path(relative_path):
            errors.append(f"Project {item.get('id')} heeft een ongeldig backupRelativePath.")
            continue
        project_root = package_root / Path(*PurePosixPath(relative_path).parts)
        if not project_root.is_dir():
            errors.append(f"Projectmap ontbreekt: {relative_path}")
            continue
        ...
        if item.get("fileCount") != actual_count:
            errors.append(
                f"Bestandsaantal project {item.get('id')} wijkt af: {actual_count} i.p.v. {item.get('fileCount')}"
            )
    ...
    try:
        ...
        for item in copied_attachments:
            ...
            if not valid_relative_path(relative_path) or not (
                ...
                errors.append(f"Gekopieerde bijlage ontbreekt: {relative_path}")
        ...
        if counts.get("attachmentsCopied") != len(copied_attachments):
            errors.append("counts.attachmentsCopied wijkt af van path-mappings.json.")
        if counts.get("attachmentsMissing") != len(missing_attachments):
            errors.append("counts.attachmentsMissing wijkt af van path-mappings.json.")
        if missing_attachments:
            warnings.append(
                f"{len(missing_attachments)} historische bijlage(n) ontbraken al aan de bron."
            )
        checks["attachmentsCopiedChecked"] = len(copied_attachments)
    except Exception as exc:
        errors.append(f"path-mappings.json is ongeldig: {exc}")
...
def main() -> int:
    ...
    if args.json:
        ...
        print("")
        print("GESLAAGD" if result["valid"] else "MISLUKT")
        print(f"Pakket: {result.get('package', args.package)}")
        checks = result.get("checks", {})
        print(
            "Threads: {} | Projecten: {} | Hashes: {}".format(
